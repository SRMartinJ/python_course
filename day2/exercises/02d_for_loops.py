...
for row in data:
    row["change"]=(row["unemployment_2014"]-row["unemployment_2009"])
    if row["change"]==0:
        print("Arbetslösheten i %s står still!" % row["municipality"])

    elif row["change"]<0:
        print("Arbetslösheten i %s minskade med %s procentenheter." %(row["municipality"], abs(row["change"])))

    elif row["change"]>0:
        print("Arbetslösheten ökade i %s. Plus %s procentenheter!" %(row["municipality"], row["change"]))
